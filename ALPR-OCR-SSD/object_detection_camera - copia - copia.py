...
ret, frame = cap.read()
roi_coords = cv2.selectROI(frame)
print(roi_coords)
print(type(roi_coords[0]))
...
while True:
        ...
        roi_cropped = image_np[int(roi_coords[1]):int(roi_coords[1]+roi_coords[3]), int(roi_coords[0]):int(roi_coords[0]+roi_coords[2])]
        ...
        cv2.putText(roi_cropped, fps_txt, (5, 20), cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,255), 1)
        ...
        input_tensor = tf.convert_to_tensor(np.expand_dims(roi_cropped, axis=0), dtype=tf.float32)
        detections, predictions_dict, shapes = detect_fn(input_tensor)

        label_id_offset = 1
        image_np_with_detections = roi_cropped.copy()
